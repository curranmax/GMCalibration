...
def compareVR(vr_data, ra):
	dists = [vr_data[i].tvec.dist(vr_data[j].tvec) for i in range(len(vr_data)) for j in range(i + 1, len(vr_data))]
	print('Max distance:', max(dists) * 1000.0, 'mm')

	angles = [ra.getVal(dp.rot_mtx) for dp in vr_data]

	angle_dif = [abs(angles[i] - angles[j]) for i in range(len(vr_data)) for j in range(i + 1, len(vr_data))]
