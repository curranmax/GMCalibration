...
class Matrix:
	...
	def dist(self, rm):
		# See https://math.stackexchange.com/questions/2113634/comparing-two-rotation-matrices
		r_ab = self * rm.transpose()

		v = (r_ab.trace() - 1.0) / 2.0
...
def reduceVRData(vr_data, k):
	...
	for i in range(0, len(vr_data), k):
		...
		this_quat = sum((dp.quat for dp in this_data), Quat(0.0, 0.0, 0.0, 0.0)).mult(1.0 / float(len(this_data)))
